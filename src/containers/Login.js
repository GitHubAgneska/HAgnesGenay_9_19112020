...
export default class Login {
  ...
  handleSubmitAdmin = e => {
    const user = {
      type: "Admin",
      email: e.target.querySelector(`input[data-testid="admin-email-input"]`).value, // corrected 
      password: e.target.querySelector(`input[data-testid="admin-password-input"]`).value, // corrected 
      status: "connected"
    }
    this.localStorage.setItem("user", JSON.stringify(user))
    const userExists = this.checkIfUserExists(user)
